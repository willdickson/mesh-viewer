<script>
  ...
  import { Filename, FileUploaderButton, Checkbox } from "carbon-components-svelte";
  import { meshChecked, colorMap } from './stores.js'
  import { getMeshColor } from './geom_utils.js'
  /**
    ...
    {#each files as { name }, i}
      <span class:bx--file__selected-file="{true}">
        ...
        <span class:bx--file__state-container="{true}">
          <Filename iconDescription="{iconDescription}" status=""/>
          <div class="padded">
            <span class="dot" style:background-color={getMeshColor(name,$colorMap)} />
          </div>
          <div class="padded">
            <Checkbox bind:checked={$meshChecked[name]}/>
          </div>
        </span>
      </span>
    {/each}
